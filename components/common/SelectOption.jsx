...
function SelectOption({ titleName, items }) {
    ...
    return (
        <>
            <div className={`dropdown dropdown-bottom dropdown-open mt-10 ${open ? "z-[2029]" : ""}`}>
                <button onClick={() => setOpen(!open)} className="btn m-1 w-[150px] lg:w-[200px] flex justify-between items-center bg-[#fff] hover:bg-[#fff]">
                    <span className='text-[#005974]'>{selectName === "" ? titleName : selectName}</span>
                ...
                <ul hidden={!open} className={`dropdown-content menu p-2 shadow-lg bg-base-100 rounded-box w-[100%]`}>
                    {
                        items?.map((item , i)=> (
                            <li key={i} onClick={handleSelect}><a value={item.value} className="p-1 px-2">{item.text}</a></li>
                        ))
                    }
                </ul>
            </div>
